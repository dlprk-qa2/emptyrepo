...
data "azurerm_subscription" "mission-critical-prod" {
}
...
resource "azurerm_container_registry" "cr-finance" {
  provider = azurerm.mission-critical-prod 
...
resource "azurerm_role_assignment" "roleassingment-finance" {
  provider = azurerm.mission-critical-prod 
  scope                = data.azurerm_subscription.mission-critical-prod.id
